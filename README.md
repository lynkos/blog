<div align="center">
  ...
  This is a fork of <a target="_blank" href="https://github.com/cotes2020/jekyll-theme-chirpy" title="Link to jekyll-theme-chirpy GitHub repository"><code>jekyll-theme-chirpy</code></a> that has been modified to include a [custom tabbed container plugin](_plugins/tabs.rb), [custom gallery slideshow plugin](_plugins/gallery.rb), auto-generates `CNAME` and `.nojekyll` files during [build and deployment](.github/workflows/pages-deploy.yml), [custom image proxy](#setup-image-proxy) (e.g. for Twitter links), [custom plugin to fix malformed Markdown links](_plugins/fix-anchor-links.rb) (originally generated by [Enveloppe](#setup-enveloppe)), auto-generates `CNAME` and `.nojekyll` files during [build and deployment](.github/workflows/pages-deploy.yml), custom "Important" prompt, and more.
</div>
